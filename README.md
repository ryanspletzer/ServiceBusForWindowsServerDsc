...
* **CertificateAutoGenerationKey**: This passphrase is required for certificate
  auto generation. This parameter is mandatory if you want certificates to be
  auto generated.
...
* **$GatewayDBConnectionStringEncrypt**: Represents whether the database server
  housing the gateway database will use SSL/TLS or not. The default value for
...
* **MessageContainerDBConnectionStringDataSource**: The database server used for
  ...
  used named instance and port info if those are being used. The default value
  for this will be the same as that specified for farm management database
  server.
...
* **TcpPort**: Represents the port that the Service Bus for Windows Server uses
  for TCP. The default is 9354. This setting cannot be changed after the farm
  has been provisioned.
* **TenantApiCredentials**: Sets the resource provider credentials for the
  tenant portal. The resource provider is a component that exposes the
  management API to the (Azure Pack) portal. There are two Service Bus
  management portals; the Admin portal (which provides a set of resource
  provider APIs for farm administration), and the Tenant portal (which is the
  Windows Azure Management Portal). Use these credentials when you manually
  install the server farm and connect to the tenant portal.
...
* **RunAsPassword**: Specifies the password for the user account under which
  ...
  service account and the security policy requires the service account password
  to be changed at regular intervals, you must perform specific actions on each
...
* **ContainerDBConnectionStringCredential**: The credential for connecting to
  the container database. Not required if integrated authentication will be
  used.
...
* **PrimaryKey**: The key that will be used by this authorization rule. If not
  provided, Service Bus generates a key. You can explicitly set this parameter
